...
import { Slide, PresentationStyle } from "@/lib/types";
...
function SlidePreview({
  ...
  const bgColor =
    style.backgroundStyle === "light"
      ? "#ffffff"
      : getHexColor(palette.darkBg);
  const textColor =
    style.backgroundStyle === "light"
      ? "#1a1a2e"
      : getHexColor(palette.textLight);
  const mutedColor =
    style.backgroundStyle === "light"
      ? "#666666"
      : getHexColor(palette.textMuted);
    ...
    return (
      <div className="flex items-center justify-center h-64 bg-gray-800/30 rounded-lg border border-gray-700 border-dashed">
        <p className="text-gray-500 text-center">
          ...
          <br />
          <span className="text-sm">after generating</span>
        </p>
      </div>
    );
  }
